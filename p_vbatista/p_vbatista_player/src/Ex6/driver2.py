...
class Driver:
    def __init__(self):
        ...
        self.biggest_area_green2 = None
        self.biggest_area_blue2 = None
        self.biggest_area_red2 = None
        self.biggest_centroid_red2 = None
        self.biggest_centroid_blue2 = None
        self.biggest_centroid_green2 = None
        self.teams = None
        ...
        # Initialize the detection of the robots through computer vision in camera 2
        self.bridge2 = CvBridge()
        self.image_subscriber2 = rospy.Subscriber('/' + self.name + '/camera_2/rgb/image_raw', Image,
                                                  self.BackdetectRobotsWithVisionCallback)
    ...
    def BackdetectRobotsWithVisionCallback(self, image_msg_2):
        # Convert the from imgmsg ROS to opencv.
        try:
            rgb_image_original2 = self.bridge2.imgmsg_to_cv2(image_msg_2, desired_encoding="bgr8")
        except CvBridgeError as e:
            print(e)

        # Segment the color for red players
        # Processing
        mask_red2 = cv2.inRange(rgb_image_original2, self.mins_red, self.maxs_red)
        # conversion from numpy from uint8 to bool
        mask_red2 = mask_red2.astype(bool)

        # Segment the color for green players
        # Processing
        mask_green2 = cv2.inRange(rgb_image_original2, self.mins_green, self.maxs_green)
        # conversion from numpy from uint8 to bool
        mask_green2 = mask_green2.astype(bool)

        # Segment the color for blue players
        # Processing
        mask_blue2 = cv2.inRange(rgb_image_original2, self.mins_blue, self.maxs_blue)
        # conversion from numpy from uint8 to bool
        mask_blue2 = mask_blue2.astype(bool)

        # Find centroids of the largest blobs of each mask
        self.biggest_centroid_red2, self.biggest_area_red2 = self.findCentroid(mask_red2.astype(np.uint8) * 255)
        self.biggest_centroid_green2, self.biggest_area_green2 = self.findCentroid(mask_green2.astype(np.uint8) * 255)
        self.biggest_centroid_blue2, self.biggest_area_blue2 = self.findCentroid(mask_blue2.astype(np.uint8) * 255)

        if self.name in self.teams['red_team']:
            if self.camera_state == 'hunting':
                if self.biggest_centroid_blue2 is not None and self.biggest_centroid_green is not None:
                    if self.biggest_area_blue2 > self.biggest_area_green:
                        self.camera_state = 'escaping'
        ...
        if self.debug is True:
            # Paint largest blobs
            rgb_image_processed2 = copy.deepcopy(rgb_image_original2)
            rgb_image_processed2 = self.paintBlobs(rgb_image_processed2, mask_red2, color='red')
            rgb_image_processed2 = self.paintBlobs(rgb_image_processed2, mask_green2, color='green')
            rgb_image_processed2 = self.paintBlobs(rgb_image_processed2, mask_blue2, color='blue')

            if self.biggest_centroid_red2 is not None:
                rgb_image_processed = cv2.circle(rgb_image_processed2,
                                                 (round(self.biggest_centroid_red2[0]),
                                                  round(self.biggest_centroid_red2[1])),
                                                 8, (0, 0, 0), -1)

            if self.biggest_centroid_green2 is not None:
                rgb_image_processed2 = cv2.circle(rgb_image_processed2,
                                                  (round(self.biggest_centroid_green2[0]),
                                                   round(self.biggest_centroid_green2[1])),
                                                  8, (0, 0, 0), -1)

            if self.biggest_centroid_blue2 is not None:
                rgb_image_processed2 = cv2.circle(rgb_image_processed2,
                                                  (round(self.biggest_centroid_blue2[0]),
                                                   round(self.biggest_centroid_blue2[1])),
                                                  8, (0, 0, 0), -1)

            # cv2.imshow('Original image', rgb_image_original2)  # Display the image
            # cv2.imshow('Processed image', rgb_image_processed2)  # Display the image
            # cv2.imshow('Red players mask', mask_red2.astype(np.uint8) * 255)  # Display the segmented image
            # cv2.imshow('Green players mask', mask_green2.astype(np.uint8) * 255)  # Display the segmented image
            # cv2.imshow('Blue players mask', mask_blue2.astype(np.uint8) * 255)  # Display the segmented image
            # cv2.waitKey(1)
    ...
    def pursuePrey(self, my_prey_color, minimum_speed=0.4, maximum_speed=0.6):
        """
        Function that receives the goal pose and calculate the speed and angle to drive to that goal.
        :param my_prey_color:
        :param minimum_speed: minimum speed allowed to the robot when driving to the goal pose.
    ...
    def escapeHunter(self, my_hunter_color, maximum_speed=1.0):
        ...
        if my_hunter_color == 'red':
            if self.biggest_centroid_red is not None or self.biggest_centroid_red2 is not None:
                if self.biggest_centroid_red is not None:
                    self.distance_to_center_hunter = round(self.biggest_centroid_red[0]) - self.image_center
                    ...
                    self.distance_to_center_hunter = round(self.biggest_centroid_red2[0]) - self.image_center
                    if self.distance_to_center_hunter > 0:  # turn left, to the opposite direction
                        self.angle = 0.6
                    elif self.distance_to_center_hunter < 0:  # turn right, to the opposite direction
                        self.angle = -0.6
            ...
            if self.biggest_centroid_green is not None or self.biggest_centroid_green2 is not None:
                if self.biggest_centroid_green is not None:
                    ...
                    self.distance_to_center_hunter = round(self.biggest_centroid_green2[0]) - self.image_center
                    if self.distance_to_center_hunter > 0:  # turn left, to the opposite direction
                        self.angle = 0.6
                    elif self.distance_to_center_hunter < 0:  # turn right, to the opposite direction
                        self.angle = -0.6
            ...
            if self.biggest_centroid_blue is not None or self.biggest_centroid_blue2 is not None:
                if self.biggest_centroid_blue is not None:
                    ...
                    self.distance_to_center_hunter = round(self.biggest_centroid_blue2[0]) - self.image_center
                    if self.distance_to_center_hunter > 0:  # turn left, to the opposite direction
                        self.angle = 0.6
                    elif self.distance_to_center_hunter < 0:  # turn right, to the opposite direction
                        self.angle = -0.6
